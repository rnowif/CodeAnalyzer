...
{
    private readonly Dictionary<IMethodSymbol, HashSet<IMethodSymbol>> _internalGraph = new(SymbolEqualityComparer.Default);
    ...
    public void AddChildren(IMethodSymbol node, IEnumerable<IMethodSymbol> children)
    {
        ...
        {
            _internalGraph[node] = new HashSet<IMethodSymbol>(SymbolEqualityComparer.Default);
        }
    ...
    private IEnumerable<IMethodSymbol> GetRecursiveChildren(IMethodSymbol method, IReadOnlyCollection<IMethodSymbol> breadcrumb)
    {
        ...
        if (breadcrumb.Contains(method, SymbolEqualityComparer.Default))
        {
    ...
    {
        var visitedNodes = new Dictionary<IMethodSymbol, bool>(SymbolEqualityComparer.Default);
